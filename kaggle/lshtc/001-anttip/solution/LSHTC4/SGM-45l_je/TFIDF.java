...
public class TFIDF implements Serializable{
	...
	//if (1==1) return;
	//
	if (use_tfidf!=1 && use_tfidf!=2 && use_tfidf!=6 && use_tfidf!=7 && use_tfidf!=13&& use_tfidf!=15&& use_tfidf!=17&& use_tfidf!=18&& use_tfidf!=19 && use_tfidf!=22 && use_tfidf!=23) return;
	...
	if (use_tfidf==6) for (float count: counts) counts[t++]= (float)Math.log(1.0+count);
	    ...
	    if (use_tfidf==15||use_tfidf==18||use_tfidf==23) for (float count: counts) counts[t++]= (float)((count*norm)/(count * norm+length_norm))* norm2;
