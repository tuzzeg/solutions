...
import java.util.HashMap; // JESSE
...
public class SGM implements Serializable{
    ...
    PrunedSetsPreProc preproc = null;

    public SGM() {
	// JESSE
	try {
	    preproc = new PrunedSetsPreProc("outCount_PS_topn.dat", "outMap_PS_topn.dat");
	} catch(Exception e) {
	    System.err.println("[Error] failed to create Preprocessing...");
	    e.printStackTrace();
	    System.exit(1);
	}
	// ESSEJ
    ...
    // JESSE
    public int[] get_pruned_set(int[] labels) {
	int[] labels2= Arrays.copyOf(labels, labels.length);
	Arrays.sort(labels2);
	return preproc.labels2setindices(labels2,0); // use 'cover' strategy
    }
    
    /*
     * Given br labels[], returns powerset labels[]
     */
	...
	//if (labels2powerset!=null) labels= get_label_powerset(labels);  // ANTTI
        labels= get_pruned_set(labels);  // JESSE
	...
	for (t= 0; t<terms.length; t++) {
	    ...
	    if (tfidf.use_tfidf==1 || tfidf.use_tfidf==3 || tfidf.use_tfidf==5|| tfidf.use_tfidf==8 || tfidf.use_tfidf==15|| tfidf.use_tfidf==16 || tfidf.use_tfidf==17|| tfidf.use_tfidf==18|| tfidf.use_tfidf==19|| tfidf.use_tfidf==23) prune-= Math.log(tfidf.get_idf(term2));
	    //if (tfidf.use_tfidf==8) 
	...
	while (entries.hasNext()) {
	    ...
	    if (tfidf.use_tfidf==1 || tfidf.use_tfidf==3 || tfidf.use_tfidf==8|| tfidf.use_tfidf==15 || tfidf.use_tfidf==16|| tfidf.use_tfidf==17|| tfidf.use_tfidf==18|| tfidf.use_tfidf==19|| tfidf.use_tfidf==23) lprob+= Math.log(tfidf.get_idf(((CountKey)entry.getKey()).term));
	...
	//System.out.println(node_sort.length+" "+label_posteriors.size()+ " "+retrieve_sort.size());
        ...
        if (model.node_links!=null && label_posteriors.size()!=0) {
	//if (model.node_links!=null) {
	    Integer count2;
	...
	//System.out.println(" "+result.indices[i]+"\t"+result.values[i]);
	//}
	
	//System.out.println("Y_ "+Arrays.toString(result.indices));
	//System.out.println("P_ "+Arrays.toString(result.values));
	
	
	HashMap<Integer,Float> recomb_result = preproc.recombination(result.indices, result.values);
	result.indices = PrunedSetsPreProc.cast(recomb_result.keySet());
	result.values = PrunedSetsPreProc.cast(recomb_result.values());
	...
	//System.out.println("Y+ "+Arrays.toString(result.indices));
	//System.out.println("P+ "+Arrays.toString(result.values));
	SparseVector tmp_result= new SparseVector(result.indices.length+1);
	tmp_result.indices[result.indices.length]= -1;
	for(int i = 0; i < result.indices.length; i++) {
	    tmp_result.indices[i]= result.indices[i];
	    tmp_result.values[i]= result.values[i];
	}
        return tmp_result;
	//The decomposition can go to the very end of inference(), and manipulate the SparseVector variables result.indices and result.values.
	//SparseVector result is the ranked sparse vector of labels and their scores.
	//return result;
    }
	...
	for (int t= 0; t<terms.length; t++) {
	    ...
	    if (tfidf.use_tfidf==1 || tfidf.use_tfidf==3 || tfidf.use_tfidf==8|| tfidf.use_tfidf==13|| tfidf.use_tfidf==15|| tfidf.use_tfidf==17|| tfidf.use_tfidf==18|| tfidf.use_tfidf==19|| tfidf.use_tfidf==20|| tfidf.use_tfidf==21|| tfidf.use_tfidf==22|| tfidf.use_tfidf==23) count*= tfidf.get_idf(term2);
	...
	if (feedback!=0) {
	    ...
	    inference_results= inference(terms, feedback_counts, doc_length, top_k, max_retrieved, -1);
	}	    
	...
	int i= 0;
	//double instantiate_threshold= 0.0000000000001; //0.5656648009561633
	//double instantiate_threshold= 0.00001; //0.5658961217981713 //0.5656648009561633
	//double instantiate_threshold= 0.0001;  //0.566652510313015  //0.5656648009561633
	//double instantiate_threshold= 0.001;   //0.5660681767853976 //0.5656648009561633
	//double instantiate_threshold= 0.1;                          //0.5688107644074708 //0.6693715540170254
	//double instantiate_threshold= 0.2;                          //0.5704184442594258 //0.6698245156894995
	//double instantiate_threshold= 0.3;                          //0.569068521917137  //0.6726650665439879
	//double instantiate_threshold= 0.4;                          //0.5698916385799485 //0.6719292112262513
	//double instantiate_threshold= 0.5;                          //0.5702254015091315 //0.6728918239136027
	//double instantiate_threshold= 0.6;                          //                   //0.6731572959560784
	//double instantiate_threshold= 0.7;                          //                   //0.6722433531431814
	//double instantiate_threshold= 0.8;                          //                   //0.6714771120285087
	//double instantiate_threshold= 1.0;                          //0.5666345961886702 //0.6690779664586323
	for (int label: inference_results.indices) {
	    //System.out.println(label+" "+inference_results.indices.length);
	    if (label==-1) continue;
	    //double score= 1.0/++i; //0.566652510313015
	    double score= Math.log(1.0/++i); 
	    //Math.log((1.0/(t+1)));
	    
	    TreeSet<DoubleBuffer> treesort= label_instances.get(label);
	    //if (treesort.size()< label_cutoffs.get(label) || score> ((DoubleBuffer)treesort.first()).get(0)) {
	    //if (treesort.size()== label_cutoffs.get(label)) treesort.pollFirst();
	...
	if (fps.size()!=fns.size()){ //missing labels, use surrogate measure
		...
		if (fn2==null) {
		    fp2= fps2.get(label);
		    while (fp2-->0) {
			label= label_list[j]; // <-- AooB Exception here // JESSE
			j= (j+1)%i;
